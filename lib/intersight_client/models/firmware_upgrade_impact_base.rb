...
module IntersightClient
  ...
  class FirmwareUpgradeImpactBase < MoBaseMo
    ...
    attr_accessor :computation_state

    attr_accessor :exclude_components
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IntersightClient::FirmwareUpgradeImpactBase` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IntersightClient::FirmwareUpgradeImpactBase`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def class_id=(class_id)
      validator = EnumAttributeValidator.new('String', ["firmware.UpgradeImpact", "firmware.UpgradeImpactStatus"])
    ...
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] computation_state Object to be assigned
    def computation_state=(computation_state)
      validator = EnumAttributeValidator.new('String', ["Inprogress", "Completed", "Unavailable"])
      unless validator.valid?(computation_state)
        fail ArgumentError, "invalid value for \"computation_state\", must be one of #{validator.allowable_values}."
      end
    ...
    def summary=(summary)
      validator = EnumAttributeValidator.new('String', ["Basic", "Detail"])
      unless validator.valid?(summary)
        fail ArgumentError, "invalid value for \"summary\", must be one of #{validator.allowable_values}."
      end
      @summary = summary
    end
    ...
    def build_from_hash(attributes)
      ...
      super(attributes)
      FirmwareUpgradeImpactBase.openapi_types.each_pair do |key, type|
        if attributes[FirmwareUpgradeImpactBase.attribute_map[key]].nil? && FirmwareUpgradeImpactBase.openapi_nullable.include?(key)
          self.send("#{key}=", nil)
        elsif type =~ /\AArray<(.*)>/i
          # check to ensure the input is an array given that the attribute
          # is documented as an array but the input is not
          if attributes[FirmwareUpgradeImpactBase.attribute_map[key]].is_a?(Array)
            self.send("#{key}=", attributes[FirmwareUpgradeImpactBase.attribute_map[key]].map { |v| _deserialize($1, v) })
          end
        elsif !attributes[FirmwareUpgradeImpactBase.attribute_map[key]].nil?
          self.send("#{key}=", _deserialize(type, attributes[FirmwareUpgradeImpactBase.attribute_map[key]]))
        end
      end
    ...
    def to_hash
      hash = super
      FirmwareUpgradeImpactBase.attribute_map.each_pair do |attr, param|
        value = self.send(attr)
        if value.nil?
          is_nullable = FirmwareUpgradeImpactBase.openapi_nullable.include?(attr)
