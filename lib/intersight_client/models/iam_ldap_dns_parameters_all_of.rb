...
module IntersightClient
  ...
  class IamLdapDnsParametersAllOf
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IntersightClient::IamLdapDnsParametersAllOf` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IntersightClient::IamLdapDnsParametersAllOf`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def class_id=(class_id)
      validator = EnumAttributeValidator.new('String', ["iam.LdapDnsParameters"])
    ...
    # @param [Object] search_domain Value to be assigned
    def search_domain=(search_domain)
      if !search_domain.nil? && search_domain.to_s.length > 64
        fail ArgumentError, 'invalid value for "search_domain", the character length must be smaller than or equal to 64.'
      end

      pattern = Regexp.new(/^$|^(([a-zA-Z0-9])|([a-zA-Z0-9][a-zA-Z0-9\.\-]*[a-zA-Z0-9]))$/)
      if !search_domain.nil? && search_domain !~ pattern
        fail ArgumentError, "invalid value for \"search_domain\", must conform to the pattern #{pattern}."
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] search_forest Value to be assigned
    def search_forest=(search_forest)
      if !search_forest.nil? && search_forest.to_s.length > 64
        fail ArgumentError, 'invalid value for "search_forest", the character length must be smaller than or equal to 64.'
      end

      pattern = Regexp.new(/^$|^(([a-zA-Z0-9])|([a-zA-Z0-9][a-zA-Z0-9\.\-]*[a-zA-Z0-9]))$/)
      if !search_forest.nil? && search_forest !~ pattern
        fail ArgumentError, "invalid value for \"search_forest\", must conform to the pattern #{pattern}."
      end
    ...
    def source=(source)
      validator = EnumAttributeValidator.new('String', ["Extracted", "Configured", "ConfiguredExtracted"])
    ...
    def build_from_hash(attributes)
      ...
      IamLdapDnsParametersAllOf.openapi_types.each_pair do |key, type|
        if attributes[IamLdapDnsParametersAllOf.attribute_map[key]].nil? && IamLdapDnsParametersAllOf.openapi_nullable.include?(key)
          self.send("#{key}=", nil)
        elsif type =~ /\AArray<(.*)>/i
          # check to ensure the input is an array given that the attribute
          # is documented as an array but the input is not
          if attributes[IamLdapDnsParametersAllOf.attribute_map[key]].is_a?(Array)
            self.send("#{key}=", attributes[IamLdapDnsParametersAllOf.attribute_map[key]].map { |v| _deserialize($1, v) })
          end
        elsif !attributes[IamLdapDnsParametersAllOf.attribute_map[key]].nil?
          self.send("#{key}=", _deserialize(type, attributes[IamLdapDnsParametersAllOf.attribute_map[key]]))
        end
      end
    ...
    def to_hash
      hash = {}
      IamLdapDnsParametersAllOf.attribute_map.each_pair do |attr, param|
        value = self.send(attr)
        if value.nil?
          is_nullable = IamLdapDnsParametersAllOf.openapi_nullable.include?(attr)
