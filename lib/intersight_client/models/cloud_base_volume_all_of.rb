...
module IntersightClient
  # Definition of the list of properties defined in 'cloud.BaseVolume', excluding properties defined in parent classes.
  class CloudBaseVolumeAllOf
    ...
    attr_accessor :instance_attachments

    attr_accessor :iops_info
    ...
    attr_accessor :volume_tags

    attr_accessor :volume_type

    attr_accessor :zone_info
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IntersightClient::CloudBaseVolumeAllOf` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IntersightClient::CloudBaseVolumeAllOf`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def class_id=(class_id)
      validator = EnumAttributeValidator.new('String', ["cloud.AwsVolume"])
    ...
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] encryption_state Object to be assigned
    def encryption_state=(encryption_state)
      validator = EnumAttributeValidator.new('String', ["Automatic", "Encrypted", "Not_Encrypted"])
      unless validator.valid?(encryption_state)
        fail ArgumentError, "invalid value for \"encryption_state\", must be one of #{validator.allowable_values}."
      end
    ...
    def state=(state)
      validator = EnumAttributeValidator.new('String', ["UnRecognized", "Pending", "Bound", "Available", "Error", "Released", "in-use", "Creating", "Deleting", "Deleted"])
    ...
    def build_from_hash(attributes)
      ...
      CloudBaseVolumeAllOf.openapi_types.each_pair do |key, type|
        if attributes[CloudBaseVolumeAllOf.attribute_map[key]].nil? && CloudBaseVolumeAllOf.openapi_nullable.include?(key)
          self.send("#{key}=", nil)
        elsif type =~ /\AArray<(.*)>/i
          # check to ensure the input is an array given that the attribute
          # is documented as an array but the input is not
          if attributes[CloudBaseVolumeAllOf.attribute_map[key]].is_a?(Array)
            self.send("#{key}=", attributes[CloudBaseVolumeAllOf.attribute_map[key]].map { |v| _deserialize($1, v) })
          end
        elsif !attributes[CloudBaseVolumeAllOf.attribute_map[key]].nil?
          self.send("#{key}=", _deserialize(type, attributes[CloudBaseVolumeAllOf.attribute_map[key]]))
        end
      end
    ...
    def to_hash
      hash = {}
      CloudBaseVolumeAllOf.attribute_map.each_pair do |attr, param|
        value = self.send(attr)
        if value.nil?
          is_nullable = CloudBaseVolumeAllOf.openapi_nullable.include?(attr)
