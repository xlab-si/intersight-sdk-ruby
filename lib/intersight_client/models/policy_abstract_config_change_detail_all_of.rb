...
module IntersightClient
  ...
  class PolicyAbstractConfigChangeDetailAllOf
    ...
    attr_accessor :config_change_flag
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IntersightClient::PolicyAbstractConfigChangeDetailAllOf` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IntersightClient::PolicyAbstractConfigChangeDetailAllOf`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def class_id=(class_id)
      validator = EnumAttributeValidator.new('String', ["chassis.ConfigChangeDetail", "fabric.ConfigChangeDetail", "server.ConfigChangeDetail"])
    ...
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] config_change_flag Object to be assigned
    def config_change_flag=(config_change_flag)
      validator = EnumAttributeValidator.new('String', ["Pending-changes", "Drift-changes"])
      unless validator.valid?(config_change_flag)
        fail ArgumentError, "invalid value for \"config_change_flag\", must be one of #{validator.allowable_values}."
      end
      @config_change_flag = config_change_flag
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] mod_status Object to be assigned
    def mod_status=(mod_status)
      validator = EnumAttributeValidator.new('String', ["None", "Created", "Modified", "Deleted"])
      unless validator.valid?(mod_status)
        fail ArgumentError, "invalid value for \"mod_status\", must be one of #{validator.allowable_values}."
      end
    ...
    def build_from_hash(attributes)
      ...
      PolicyAbstractConfigChangeDetailAllOf.openapi_types.each_pair do |key, type|
        if attributes[PolicyAbstractConfigChangeDetailAllOf.attribute_map[key]].nil? && PolicyAbstractConfigChangeDetailAllOf.openapi_nullable.include?(key)
          self.send("#{key}=", nil)
        elsif type =~ /\AArray<(.*)>/i
          # check to ensure the input is an array given that the attribute
          # is documented as an array but the input is not
          if attributes[PolicyAbstractConfigChangeDetailAllOf.attribute_map[key]].is_a?(Array)
            self.send("#{key}=", attributes[PolicyAbstractConfigChangeDetailAllOf.attribute_map[key]].map { |v| _deserialize($1, v) })
          end
        elsif !attributes[PolicyAbstractConfigChangeDetailAllOf.attribute_map[key]].nil?
          self.send("#{key}=", _deserialize(type, attributes[PolicyAbstractConfigChangeDetailAllOf.attribute_map[key]]))
        end
      end
    ...
    def to_hash
      hash = {}
      PolicyAbstractConfigChangeDetailAllOf.attribute_map.each_pair do |attr, param|
        value = self.send(attr)
        if value.nil?
          is_nullable = PolicyAbstractConfigChangeDetailAllOf.openapi_nullable.include?(attr)
