...
module IntersightClient
  # Definition of the list of properties defined in 'hyperflex.ServiceAuthToken', excluding properties defined in parent classes.
  class HyperflexServiceAuthTokenAllOf
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IntersightClient::HyperflexServiceAuthTokenAllOf` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IntersightClient::HyperflexServiceAuthTokenAllOf`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def class_id=(class_id)
      validator = EnumAttributeValidator.new('String', ["hyperflex.ServiceAuthToken"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["Unknown", "Claiming", "Claimed", "FailedToClaim", "Revoking", "Revoked", "FailedToRevoke"])
    ...
    def build_from_hash(attributes)
      ...
      HyperflexServiceAuthTokenAllOf.openapi_types.each_pair do |key, type|
        if attributes[HyperflexServiceAuthTokenAllOf.attribute_map[key]].nil? && HyperflexServiceAuthTokenAllOf.openapi_nullable.include?(key)
          self.send("#{key}=", nil)
        elsif type =~ /\AArray<(.*)>/i
          # check to ensure the input is an array given that the attribute
          # is documented as an array but the input is not
          if attributes[HyperflexServiceAuthTokenAllOf.attribute_map[key]].is_a?(Array)
            self.send("#{key}=", attributes[HyperflexServiceAuthTokenAllOf.attribute_map[key]].map { |v| _deserialize($1, v) })
          end
        elsif !attributes[HyperflexServiceAuthTokenAllOf.attribute_map[key]].nil?
          self.send("#{key}=", _deserialize(type, attributes[HyperflexServiceAuthTokenAllOf.attribute_map[key]]))
        end
      end
    ...
    def to_hash
      hash = {}
      HyperflexServiceAuthTokenAllOf.attribute_map.each_pair do |attr, param|
        value = self.send(attr)
        if value.nil?
          is_nullable = HyperflexServiceAuthTokenAllOf.openapi_nullable.include?(attr)
