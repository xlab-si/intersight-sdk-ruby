...
module IntersightClient
  ...
  class EquipmentSwitchCardAllOf
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IntersightClient::EquipmentSwitchCardAllOf` initialize method"
      end
    ...
    def build_from_hash(attributes)
      ...
      EquipmentSwitchCardAllOf.openapi_types.each_pair do |key, type|
        if attributes[EquipmentSwitchCardAllOf.attribute_map[key]].nil? && EquipmentSwitchCardAllOf.openapi_nullable.include?(key)
          self.send("#{key}=", nil)
        elsif type =~ /\AArray<(.*)>/i
          # check to ensure the input is an array given that the attribute
          # is documented as an array but the input is not
          if attributes[EquipmentSwitchCardAllOf.attribute_map[key]].is_a?(Array)
            self.send("#{key}=", attributes[EquipmentSwitchCardAllOf.attribute_map[key]].map { |v| _deserialize($1, v) })
          end
        elsif !attributes[EquipmentSwitchCardAllOf.attribute_map[key]].nil?
          self.send("#{key}=", _deserialize(type, attributes[EquipmentSwitchCardAllOf.attribute_map[key]]))
        end
      end
    ...
    def to_hash
      hash = {}
      EquipmentSwitchCardAllOf.attribute_map.each_pair do |attr, param|
        value = self.send(attr)
        if value.nil?
          is_nullable = EquipmentSwitchCardAllOf.openapi_nullable.include?(attr)
