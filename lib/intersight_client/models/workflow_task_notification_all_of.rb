...
module IntersightClient
  # Definition of the list of properties defined in 'workflow.TaskNotification', excluding properties defined in parent classes.
  class WorkflowTaskNotificationAllOf
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IntersightClient::WorkflowTaskNotificationAllOf` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IntersightClient::WorkflowTaskNotificationAllOf`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def build_from_hash(attributes)
      ...
      WorkflowTaskNotificationAllOf.openapi_types.each_pair do |key, type|
        if attributes[WorkflowTaskNotificationAllOf.attribute_map[key]].nil? && WorkflowTaskNotificationAllOf.openapi_nullable.include?(key)
          self.send("#{key}=", nil)
        elsif type =~ /\AArray<(.*)>/i
          # check to ensure the input is an array given that the attribute
          # is documented as an array but the input is not
          if attributes[WorkflowTaskNotificationAllOf.attribute_map[key]].is_a?(Array)
            self.send("#{key}=", attributes[WorkflowTaskNotificationAllOf.attribute_map[key]].map { |v| _deserialize($1, v) })
          end
        elsif !attributes[WorkflowTaskNotificationAllOf.attribute_map[key]].nil?
          self.send("#{key}=", _deserialize(type, attributes[WorkflowTaskNotificationAllOf.attribute_map[key]]))
        end
      end
    ...
    def to_hash
      hash = {}
      WorkflowTaskNotificationAllOf.attribute_map.each_pair do |attr, param|
        value = self.send(attr)
        if value.nil?
          is_nullable = WorkflowTaskNotificationAllOf.openapi_nullable.include?(attr)
