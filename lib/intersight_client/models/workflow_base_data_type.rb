...
module IntersightClient
  ...
  class WorkflowBaseDataType < MoBaseComplexType
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IntersightClient::WorkflowBaseDataType` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IntersightClient::WorkflowBaseDataType`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def class_id=(class_id)
      validator = EnumAttributeValidator.new('String', ["workflow.ArrayDataType", "workflow.CustomDataType", "workflow.MoReferenceDataType", "workflow.PrimitiveDataType", "workflow.TargetDataType"])
    ...
    def label=(label)
      if !label.nil? && label.to_s.length > 92
        fail ArgumentError, 'invalid value for "label", the character length must be smaller than or equal to 92.'
      end
    ...
    def name=(name)
      if !name.nil? && name.to_s.length > 92
        fail ArgumentError, 'invalid value for "name", the character length must be smaller than or equal to 92.'
      end
    ...
    def build_from_hash(attributes)
      ...
      super(attributes)
      WorkflowBaseDataType.openapi_types.each_pair do |key, type|
        if attributes[WorkflowBaseDataType.attribute_map[key]].nil? && WorkflowBaseDataType.openapi_nullable.include?(key)
          self.send("#{key}=", nil)
        elsif type =~ /\AArray<(.*)>/i
          # check to ensure the input is an array given that the attribute
          # is documented as an array but the input is not
          if attributes[WorkflowBaseDataType.attribute_map[key]].is_a?(Array)
            self.send("#{key}=", attributes[WorkflowBaseDataType.attribute_map[key]].map { |v| _deserialize($1, v) })
          end
        elsif !attributes[WorkflowBaseDataType.attribute_map[key]].nil?
          self.send("#{key}=", _deserialize(type, attributes[WorkflowBaseDataType.attribute_map[key]]))
        end
      end
    ...
    def to_hash
      hash = super
      WorkflowBaseDataType.attribute_map.each_pair do |attr, param|
        value = self.send(attr)
        if value.nil?
          is_nullable = WorkflowBaseDataType.openapi_nullable.include?(attr)
