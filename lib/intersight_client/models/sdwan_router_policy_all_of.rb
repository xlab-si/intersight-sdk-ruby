...
module IntersightClient
  ...
  class SdwanRouterPolicyAllOf
    ...
    attr_accessor :wan_termination_type
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IntersightClient::SdwanRouterPolicyAllOf` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IntersightClient::SdwanRouterPolicyAllOf`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def class_id=(class_id)
      validator = EnumAttributeValidator.new('String', ["sdwan.RouterPolicy"])
    ...
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] deployment_size Object to be assigned
    def deployment_size=(deployment_size)
      validator = EnumAttributeValidator.new('String', ["Typical", "Minimal"])
      unless validator.valid?(deployment_size)
        fail ArgumentError, "invalid value for \"deployment_size\", must be one of #{validator.allowable_values}."
      end
      @deployment_size = deployment_size
    end

    # Custom attribute writer method with validation
    # @param [Object] wan_count Value to be assigned
    def wan_count=(wan_count)
      if !wan_count.nil? && wan_count > 3
        fail ArgumentError, 'invalid value for "wan_count", must be smaller than or equal to 3.'
      end

      if !wan_count.nil? && wan_count < 2
        fail ArgumentError, 'invalid value for "wan_count", must be greater than or equal to 2.'
      end
    ...
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] wan_termination_type Object to be assigned
    def wan_termination_type=(wan_termination_type)
      validator = EnumAttributeValidator.new('String', ["Single", "Dual"])
      unless validator.valid?(wan_termination_type)
        fail ArgumentError, "invalid value for \"wan_termination_type\", must be one of #{validator.allowable_values}."
      end
      @wan_termination_type = wan_termination_type
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          profiles == o.profiles &&
          solution_image == o.solution_image
    end
    ...
    def build_from_hash(attributes)
      ...
      SdwanRouterPolicyAllOf.openapi_types.each_pair do |key, type|
        if attributes[SdwanRouterPolicyAllOf.attribute_map[key]].nil? && SdwanRouterPolicyAllOf.openapi_nullable.include?(key)
          self.send("#{key}=", nil)
        elsif type =~ /\AArray<(.*)>/i
          # check to ensure the input is an array given that the attribute
          # is documented as an array but the input is not
          if attributes[SdwanRouterPolicyAllOf.attribute_map[key]].is_a?(Array)
            self.send("#{key}=", attributes[SdwanRouterPolicyAllOf.attribute_map[key]].map { |v| _deserialize($1, v) })
          end
        elsif !attributes[SdwanRouterPolicyAllOf.attribute_map[key]].nil?
          self.send("#{key}=", _deserialize(type, attributes[SdwanRouterPolicyAllOf.attribute_map[key]]))
        end
      end
    ...
    def to_hash
      hash = {}
      SdwanRouterPolicyAllOf.attribute_map.each_pair do |attr, param|
        value = self.send(attr)
        if value.nil?
          is_nullable = SdwanRouterPolicyAllOf.openapi_nullable.include?(attr)
