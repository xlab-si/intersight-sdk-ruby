...
module IntersightClient
  ...
  class BootPrecisionPolicy < PolicyAbstractPolicy
    ...
    attr_accessor :enforce_uefi_secure_boot
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IntersightClient::BootPrecisionPolicy` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IntersightClient::BootPrecisionPolicy`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def class_id=(class_id)
      validator = EnumAttributeValidator.new('String', ["boot.PrecisionPolicy"])
    ...
    # @param [Object] boot_devices Value to be assigned
    def boot_devices=(boot_devices)
      if !boot_devices.nil? && boot_devices.length > 255
        fail ArgumentError, 'invalid value for "boot_devices", number of items must be less than or equal to 255.'
      end

      if !boot_devices.nil? && boot_devices.length < 1
        fail ArgumentError, 'invalid value for "boot_devices", number of items must be greater than or equal to 1.'
      end
    ...
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] configured_boot_mode Object to be assigned
    def configured_boot_mode=(configured_boot_mode)
      validator = EnumAttributeValidator.new('String', ["Legacy", "Uefi"])
      unless validator.valid?(configured_boot_mode)
        fail ArgumentError, "invalid value for \"configured_boot_mode\", must be one of #{validator.allowable_values}."
      end
    ...
    def build_from_hash(attributes)
      ...
      super(attributes)
      BootPrecisionPolicy.openapi_types.each_pair do |key, type|
        if attributes[BootPrecisionPolicy.attribute_map[key]].nil? && BootPrecisionPolicy.openapi_nullable.include?(key)
          self.send("#{key}=", nil)
        elsif type =~ /\AArray<(.*)>/i
          # check to ensure the input is an array given that the attribute
          # is documented as an array but the input is not
          if attributes[BootPrecisionPolicy.attribute_map[key]].is_a?(Array)
            self.send("#{key}=", attributes[BootPrecisionPolicy.attribute_map[key]].map { |v| _deserialize($1, v) })
          end
        elsif !attributes[BootPrecisionPolicy.attribute_map[key]].nil?
          self.send("#{key}=", _deserialize(type, attributes[BootPrecisionPolicy.attribute_map[key]]))
        end
      end
    ...
    def to_hash
      hash = super
      BootPrecisionPolicy.attribute_map.each_pair do |attr, param|
        value = self.send(attr)
        if value.nil?
          is_nullable = BootPrecisionPolicy.openapi_nullable.include?(attr)
