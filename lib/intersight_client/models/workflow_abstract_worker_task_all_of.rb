...
module IntersightClient
  ...
  class WorkflowAbstractWorkerTaskAllOf
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IntersightClient::WorkflowAbstractWorkerTaskAllOf` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IntersightClient::WorkflowAbstractWorkerTaskAllOf`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      class_id_validator = EnumAttributeValidator.new('String', ["workflow.SubWorkflowTask", "workflow.WaitTask", "workflow.WorkerTask"])
      return false unless class_id_validator.valid?(@class_id)
      return false if @object_type.nil?
      object_type_validator = EnumAttributeValidator.new('String', ["workflow.SubWorkflowTask", "workflow.WaitTask", "workflow.WorkerTask"])
    ...
    def class_id=(class_id)
      validator = EnumAttributeValidator.new('String', ["workflow.SubWorkflowTask", "workflow.WaitTask", "workflow.WorkerTask"])
    ...
    def hash
      [class_id, object_type, input_parameters, on_failure, on_success, rollback_disabled, use_default].hash
    end
    ...
    def build_from_hash(attributes)
      ...
      WorkflowAbstractWorkerTaskAllOf.openapi_types.each_pair do |key, type|
        if attributes[WorkflowAbstractWorkerTaskAllOf.attribute_map[key]].nil? && WorkflowAbstractWorkerTaskAllOf.openapi_nullable.include?(key)
          self.send("#{key}=", nil)
        elsif type =~ /\AArray<(.*)>/i
          # check to ensure the input is an array given that the attribute
          # is documented as an array but the input is not
          if attributes[WorkflowAbstractWorkerTaskAllOf.attribute_map[key]].is_a?(Array)
            self.send("#{key}=", attributes[WorkflowAbstractWorkerTaskAllOf.attribute_map[key]].map { |v| _deserialize($1, v) })
          end
        elsif !attributes[WorkflowAbstractWorkerTaskAllOf.attribute_map[key]].nil?
          self.send("#{key}=", _deserialize(type, attributes[WorkflowAbstractWorkerTaskAllOf.attribute_map[key]]))
        end
      end
    ...
    def to_hash
      hash = {}
      WorkflowAbstractWorkerTaskAllOf.attribute_map.each_pair do |attr, param|
        value = self.send(attr)
        if value.nil?
          is_nullable = WorkflowAbstractWorkerTaskAllOf.openapi_nullable.include?(attr)
