...
module IntersightClient
  ...
  class NiatelemetryEpg < MoBaseMo
    ...
    def build_from_hash(attributes)
      ...
      super(attributes)
      NiatelemetryEpg.openapi_types.each_pair do |key, type|
        if attributes[NiatelemetryEpg.attribute_map[key]].nil? && NiatelemetryEpg.openapi_nullable.include?(key)
          self.send("#{key}=", nil)
        elsif type =~ /\AArray<(.*)>/i
          # check to ensure the input is an array given that the attribute
          # is documented as an array but the input is not
          if attributes[NiatelemetryEpg.attribute_map[key]].is_a?(Array)
            self.send("#{key}=", attributes[NiatelemetryEpg.attribute_map[key]].map { |v| _deserialize($1, v) })
          end
        elsif !attributes[NiatelemetryEpg.attribute_map[key]].nil?
          self.send("#{key}=", _deserialize(type, attributes[NiatelemetryEpg.attribute_map[key]]))
        end
      end
    ...
    def to_hash
      hash = super
      NiatelemetryEpg.attribute_map.each_pair do |attr, param|
        value = self.send(attr)
        if value.nil?
          is_nullable = NiatelemetryEpg.openapi_nullable.include?(attr)
