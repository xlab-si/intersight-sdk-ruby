...
module IntersightClient
  ...
  class SnmpPolicyAllOf
    ...
    def build_from_hash(attributes)
      ...
      SnmpPolicyAllOf.openapi_types.each_pair do |key, type|
        if attributes[SnmpPolicyAllOf.attribute_map[key]].nil? && SnmpPolicyAllOf.openapi_nullable.include?(key)
          self.send("#{key}=", nil)
        elsif type =~ /\AArray<(.*)>/i
          # check to ensure the input is an array given that the attribute
          # is documented as an array but the input is not
          if attributes[SnmpPolicyAllOf.attribute_map[key]].is_a?(Array)
            self.send("#{key}=", attributes[SnmpPolicyAllOf.attribute_map[key]].map { |v| _deserialize($1, v) })
          end
        elsif !attributes[SnmpPolicyAllOf.attribute_map[key]].nil?
          self.send("#{key}=", _deserialize(type, attributes[SnmpPolicyAllOf.attribute_map[key]]))
        end
      end
    ...
    def to_hash
      hash = {}
      SnmpPolicyAllOf.attribute_map.each_pair do |attr, param|
        value = self.send(attr)
        if value.nil?
          is_nullable = SnmpPolicyAllOf.openapi_nullable.include?(attr)
