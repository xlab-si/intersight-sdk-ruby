...
module IntersightClient
  ...
  class FabricPortOperationAllOf
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IntersightClient::FabricPortOperationAllOf` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IntersightClient::FabricPortOperationAllOf`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'class_id')
        ...
        self.class_id = 'fabric.PortOperation'
      end

      if attributes.key?(:'object_type')
        self.object_type = attributes[:'object_type']
      else
        self.object_type = 'fabric.PortOperation'
      end
    ...
    def valid?
      ...
      class_id_validator = EnumAttributeValidator.new('String', ["fabric.PortOperation"])
      return false unless class_id_validator.valid?(@class_id)
      return false if @object_type.nil?
      object_type_validator = EnumAttributeValidator.new('String', ["fabric.PortOperation"])
    ...
    def class_id=(class_id)
      validator = EnumAttributeValidator.new('String', ["fabric.PortOperation"])
    ...
    def hash
      [class_id, object_type, admin_state, config_state, network_element].hash
    end
    ...
    def build_from_hash(attributes)
      ...
      FabricPortOperationAllOf.openapi_types.each_pair do |key, type|
        if attributes[FabricPortOperationAllOf.attribute_map[key]].nil? && FabricPortOperationAllOf.openapi_nullable.include?(key)
          self.send("#{key}=", nil)
        elsif type =~ /\AArray<(.*)>/i
          # check to ensure the input is an array given that the attribute
          # is documented as an array but the input is not
          if attributes[FabricPortOperationAllOf.attribute_map[key]].is_a?(Array)
            self.send("#{key}=", attributes[FabricPortOperationAllOf.attribute_map[key]].map { |v| _deserialize($1, v) })
          end
        elsif !attributes[FabricPortOperationAllOf.attribute_map[key]].nil?
          self.send("#{key}=", _deserialize(type, attributes[FabricPortOperationAllOf.attribute_map[key]]))
        end
      end
    ...
    def to_hash
      hash = {}
      FabricPortOperationAllOf.attribute_map.each_pair do |attr, param|
        value = self.send(attr)
        if value.nil?
          is_nullable = FabricPortOperationAllOf.openapi_nullable.include?(attr)
