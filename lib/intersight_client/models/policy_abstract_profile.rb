...
module IntersightClient
  ...
  class PolicyAbstractProfile < MoBaseMo
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IntersightClient::PolicyAbstractProfile` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IntersightClient::PolicyAbstractProfile`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def class_id=(class_id)
      validator = EnumAttributeValidator.new('String', ["chassis.IomProfile", "chassis.Profile", "fabric.SwitchClusterProfile", "fabric.SwitchProfile", "hyperflex.ClusterProfile", "hyperflex.NodeProfile", "kubernetes.AciCniProfile", "kubernetes.BaremetalNodeProfile", "kubernetes.ClusterProfile", "kubernetes.NodeGroupProfile", "kubernetes.VirtualMachineNodeProfile", "recovery.BackupProfile", "sdwan.Profile", "server.Profile", "server.ProfileTemplate"])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["instance"])
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          name == o.name &&
          type == o.type &&
          src_template == o.src_template && super(o)
    end
    ...
    def build_from_hash(attributes)
      ...
      super(attributes)
      PolicyAbstractProfile.openapi_types.each_pair do |key, type|
        if attributes[PolicyAbstractProfile.attribute_map[key]].nil? && PolicyAbstractProfile.openapi_nullable.include?(key)
          self.send("#{key}=", nil)
        elsif type =~ /\AArray<(.*)>/i
          # check to ensure the input is an array given that the attribute
          # is documented as an array but the input is not
          if attributes[PolicyAbstractProfile.attribute_map[key]].is_a?(Array)
            self.send("#{key}=", attributes[PolicyAbstractProfile.attribute_map[key]].map { |v| _deserialize($1, v) })
          end
        elsif !attributes[PolicyAbstractProfile.attribute_map[key]].nil?
          self.send("#{key}=", _deserialize(type, attributes[PolicyAbstractProfile.attribute_map[key]]))
        end
      end
    ...
    def to_hash
      hash = super
      PolicyAbstractProfile.attribute_map.each_pair do |attr, param|
        value = self.send(attr)
        if value.nil?
          is_nullable = PolicyAbstractProfile.openapi_nullable.include?(attr)
