...
module IntersightClient
  ...
  class ConnectorFileMessageAllOf
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IntersightClient::ConnectorFileMessageAllOf` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IntersightClient::ConnectorFileMessageAllOf`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'class_id')
        ...
        self.class_id = 'connector.FileMessage'
      end

      if attributes.key?(:'object_type')
        self.object_type = attributes[:'object_type']
      else
        self.object_type = 'connector.FileMessage'
      end

      if attributes.key?(:'msg_type')
        self.msg_type = attributes[:'msg_type']
      else
        self.msg_type = 'OpenFile'
      end
    ...
    def build_from_hash(attributes)
      ...
      ConnectorFileMessageAllOf.openapi_types.each_pair do |key, type|
        if attributes[ConnectorFileMessageAllOf.attribute_map[key]].nil? && ConnectorFileMessageAllOf.openapi_nullable.include?(key)
          self.send("#{key}=", nil)
        elsif type =~ /\AArray<(.*)>/i
          # check to ensure the input is an array given that the attribute
          # is documented as an array but the input is not
          if attributes[ConnectorFileMessageAllOf.attribute_map[key]].is_a?(Array)
            self.send("#{key}=", attributes[ConnectorFileMessageAllOf.attribute_map[key]].map { |v| _deserialize($1, v) })
          end
        elsif !attributes[ConnectorFileMessageAllOf.attribute_map[key]].nil?
          self.send("#{key}=", _deserialize(type, attributes[ConnectorFileMessageAllOf.attribute_map[key]]))
        end
      end
    ...
    def to_hash
      hash = {}
      ConnectorFileMessageAllOf.attribute_map.each_pair do |attr, param|
        value = self.send(attr)
        if value.nil?
          is_nullable = ConnectorFileMessageAllOf.openapi_nullable.include?(attr)
