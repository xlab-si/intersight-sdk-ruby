...
> <Array(nil, Integer, Hash)> delete_vmedia_policy_with_http_info(moid)
...
begin
  # Read a 'vmedia.Policy' resource.
  result = api_instance.get_vmedia_policy_list(opts)
  p result
rescue IntersightClient::ApiError => e
  puts "Error when calling VmediaApi->get_vmedia_policy_list: #{e}"
end
```

#### Using the get_vmedia_policy_list_with_http_info variant

This returns an Array which contains the response data, status code and headers.

> <Array(<VmediaPolicyResponse>, Integer, Hash)> get_vmedia_policy_list_with_http_info(opts)

```ruby
begin
  # Read a 'vmedia.Policy' resource.
  data, status_code, headers = api_instance.get_vmedia_policy_list_with_http_info(opts)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => <VmediaPolicyResponse>
rescue IntersightClient::ApiError => e
  puts "Error when calling VmediaApi->get_vmedia_policy_list_with_http_info: #{e}"
end
```
