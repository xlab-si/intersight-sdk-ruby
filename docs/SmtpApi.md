...
#### Using the get_smtp_policy_by_moid_with_http_info variant
...
#### Using the get_smtp_policy_list_with_http_info variant

This returns an Array which contains the response data, status code and headers.

> <Array(<SmtpPolicyResponse>, Integer, Hash)> get_smtp_policy_list_with_http_info(opts)

```ruby
begin
  # Read a 'smtp.Policy' resource.
  data, status_code, headers = api_instance.get_smtp_policy_list_with_http_info(opts)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => <SmtpPolicyResponse>
rescue IntersightClient::ApiError => e
  puts "Error when calling SmtpApi->get_smtp_policy_list_with_http_info: #{e}"
end
```
...
#### Using the patch_smtp_policy_with_http_info variant
