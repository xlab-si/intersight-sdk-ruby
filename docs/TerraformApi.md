...
> <TerraformExecutorResponse> get_terraform_executor_list(opts)
...
begin
  # Read a 'terraform.Executor' resource.
  result = api_instance.get_terraform_executor_list(opts)
  p result
rescue IntersightClient::ApiError => e
  puts "Error when calling TerraformApi->get_terraform_executor_list: #{e}"
end
```

#### Using the get_terraform_executor_list_with_http_info variant

This returns an Array which contains the response data, status code and headers.

> <Array(<TerraformExecutorResponse>, Integer, Hash)> get_terraform_executor_list_with_http_info(opts)

```ruby
begin
  # Read a 'terraform.Executor' resource.
  data, status_code, headers = api_instance.get_terraform_executor_list_with_http_info(opts)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => <TerraformExecutorResponse>
rescue IntersightClient::ApiError => e
  puts "Error when calling TerraformApi->get_terraform_executor_list_with_http_info: #{e}"
end
```
...
> <TerraformExecutor> update_terraform_executor(moid, terraform_executor, opts)
