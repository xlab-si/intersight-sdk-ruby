...
> <Array(nil, Integer, Hash)> delete_sdcard_policy_with_http_info(moid)
...
begin
  # Read a 'sdcard.Policy' resource.
  result = api_instance.get_sdcard_policy_list(opts)
  p result
rescue IntersightClient::ApiError => e
  puts "Error when calling SdcardApi->get_sdcard_policy_list: #{e}"
end
```

#### Using the get_sdcard_policy_list_with_http_info variant

This returns an Array which contains the response data, status code and headers.

> <Array(<SdcardPolicyResponse>, Integer, Hash)> get_sdcard_policy_list_with_http_info(opts)

```ruby
begin
  # Read a 'sdcard.Policy' resource.
  data, status_code, headers = api_instance.get_sdcard_policy_list_with_http_info(opts)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => <SdcardPolicyResponse>
rescue IntersightClient::ApiError => e
  puts "Error when calling SdcardApi->get_sdcard_policy_list_with_http_info: #{e}"
end
```
