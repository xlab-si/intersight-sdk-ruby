...
#### Using the get_vnc_console_list_with_http_info variant

This returns an Array which contains the response data, status code and headers.

> <Array(<VncConsoleResponse>, Integer, Hash)> get_vnc_console_list_with_http_info(opts)

```ruby
begin
  # Read a 'vnc.Console' resource.
  data, status_code, headers = api_instance.get_vnc_console_list_with_http_info(opts)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => <VncConsoleResponse>
rescue IntersightClient::ApiError => e
  puts "Error when calling VncApi->get_vnc_console_list_with_http_info: #{e}"
end
```
...
#### Using the patch_vnc_console_with_http_info variant
