...
> <CrdCustomResourceResponse> get_crd_custom_resource_list(opts)
...
begin
  # Read a 'crd.CustomResource' resource.
  result = api_instance.get_crd_custom_resource_list(opts)
  p result
rescue IntersightClient::ApiError => e
  puts "Error when calling CrdApi->get_crd_custom_resource_list: #{e}"
end
```

#### Using the get_crd_custom_resource_list_with_http_info variant

This returns an Array which contains the response data, status code and headers.

> <Array(<CrdCustomResourceResponse>, Integer, Hash)> get_crd_custom_resource_list_with_http_info(opts)

```ruby
begin
  # Read a 'crd.CustomResource' resource.
  data, status_code, headers = api_instance.get_crd_custom_resource_list_with_http_info(opts)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => <CrdCustomResourceResponse>
rescue IntersightClient::ApiError => e
  puts "Error when calling CrdApi->get_crd_custom_resource_list_with_http_info: #{e}"
end
```
...
> <CrdCustomResource> update_crd_custom_resource(moid, crd_custom_resource, opts)
