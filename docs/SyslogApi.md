...
> <Array(nil, Integer, Hash)> delete_syslog_policy_with_http_info(moid)
...
begin
  # Read a 'syslog.Policy' resource.
  result = api_instance.get_syslog_policy_list(opts)
  p result
rescue IntersightClient::ApiError => e
  puts "Error when calling SyslogApi->get_syslog_policy_list: #{e}"
end
```

#### Using the get_syslog_policy_list_with_http_info variant

This returns an Array which contains the response data, status code and headers.

> <Array(<SyslogPolicyResponse>, Integer, Hash)> get_syslog_policy_list_with_http_info(opts)

```ruby
begin
  # Read a 'syslog.Policy' resource.
  data, status_code, headers = api_instance.get_syslog_policy_list_with_http_info(opts)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => <SyslogPolicyResponse>
rescue IntersightClient::ApiError => e
  puts "Error when calling SyslogApi->get_syslog_policy_list_with_http_info: #{e}"
end
```
